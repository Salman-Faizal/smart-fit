...
connectDB().then(async () => {
  ...
  try {
    ...
    console.error("[startup] Failed to build search index:", err.message);
  }
...
cron.schedule("0 */6 * * *", () => {
  updateTrendingScores().catch((err) => {
    process.stderr.write(`[cron] Failed to update trending scores: ${err.message}\n`);
  });
});
