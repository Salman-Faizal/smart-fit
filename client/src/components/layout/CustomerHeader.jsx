...
export default function CustomerHeader({ cartCount = 0 }) {
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur ${
        scrolled ? "shadow-sm" : ""
      }`}
    >
          ...
          <nav className="flex items-center gap-6">
            {NAV_ITEMS.map((item) => (
              <button
                ...
                className="text-sm font-medium text-slate-500 transition hover:text-amber-600 hover:scale-[1.03]"
              >
                {item.label}
              </button>
            ))}
